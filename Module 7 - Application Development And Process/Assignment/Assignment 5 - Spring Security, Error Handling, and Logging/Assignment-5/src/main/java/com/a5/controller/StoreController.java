...
import com.a5.service.StoreService;
import com.a5.service.UserService;

@Controller
public class StoreController {
	
	// use for logging for tracing the errors and displaying
	...
	// display the store addition form
	...
	public String saveStore(@ModelAttribute("store") Store store, Model model, Principal principal) {
		logger.info("before calling Service to save store info");
	...
	public String updateStore(@ModelAttribute("store") Store store, @RequestParam long sid, Model model) {
		logger.info("before calling Service to fetch selected store detail for updating");
		...
		logger.info("after calling Service to update selected store detail under addStore method");
		return "addStore";
	}

	@GetMapping("delete")
	public String deleteStore(@RequestParam long sid) {
		logger.info("before calling Service to delete store");
		s_Service.deleteStore(sid);
		logger.info("after calling Service to delete store");
